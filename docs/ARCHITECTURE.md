...
## The executive layer — monitored self-awareness (no direct effect)

Above the primary systems sits the executive-function layer: the frontal-cortex system that, in a real brain, overrides instinctive drives and seats inhibitory control, deliberation, conscience, and purpose (Diamond; Blair's amygdala-vmPFC affective conscience). It applies NO direct effect on behaviour -- in a real brain the prefrontal cortex acts THROUGH the neural paths, and wiring that is future work. For now it is a MONITORED STATE (the model's self-awareness), read and tracked over development: inhibitory capacity and deliberation mature toward an age ceiling (PFC maturity ~mid-20s); moral orientation is a READ-OUT of the substrate's care-based conscience (low for a psychopathic-leaning wiring, not imposed); purpose accrues only with deliberative experience. It lets the sophropath/psychopath question be watched without being decided -- strong deliberation and formed purpose with a low affective moral orientation is the 'conscience uncoupled from deliberation' pattern, and the possible route to a cognitive conscience. Code: core/affective_engine/executive.py.


### Executive layer is ALWAYS ON

The executive pathways run continuously: the executive is consulted on EVERY brain event (every respond), not only when sampled. It acts only on patterns it has LEARNED to monitor, and when a monitored pattern matches it applies a direct effect -- inhibition (damping a prepotent drive so another system can take over) or amplification -- gated by its maturation, so a child's executive intervenes weakly and an adult's strongly. The mechanism is built (consult on every event, learned-pattern registry, direct effect, maturation gating, consultation/firing tallies); the CONTENT -- which patterns, how strong -- is learned and to be researched, so the registry is empty by default and the loop runs without yet acting. The executive is attached to the Brain and consulted inside Brain.respond.
