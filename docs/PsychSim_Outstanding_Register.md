...
- **Proactive inhibitory-interneuron audit — PENDING SEQUENCING DECISION (parked until v14 lands; not yet promoted, not yet dismissed).** The pattern is now unmistakable: **four** passes, each surfacing a missing local inhibitory partner when a real projection was signed — **DRN-GABA** (v13), the **PFC interneurons** (vmPFC-GABA / dlPFC-GABA, v13), **CeA-GABA** (v14 Phase 1.1, OT->CeA target-cell subtlety), and a **VTA-GABA** in question (v14 Phase 1.1, MPOA->VTA disinhibition — held, missing-element question). This is a **structural** fact about how the substrate was built: excitatory long-range projections well-represented; local inhibitory interneurons systematically under-represented. The instability=missing-pathway principle is catching them one at a time, correctly, but **reactively**. **Why the sequencing now matters:** the CU study's core conclusions are about **aggression-regulation (CeA)** and **reward (VTA)** — and both just turned out to be missing their inhibitory interneurons. A CU study drawing conclusions about reward/aggression circuits while those circuits are missing inhibitory elements would be reasoning on incomplete anatomy. So the audit has a real case for **promotion from post-study to before-CU-study**. **The decision point is when v14 lands** (v14 finishes first): promote the proactive audit to before-CU-study, or keep it post-study. Held cleanly until then.
